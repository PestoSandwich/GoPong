...
class MiniMax:
    ...
    def get_clone(self):
        minimax = MiniMax(self.max_depth, self.transpositions)
...
class IterativeMiniMax:
    ...
    def IterativeDeepening(self, player_1, player_2, max_seconds):
        start_time = dt.datetime.now()
        best_reward = -2000
        best_action_list = []
        iteration_depth = 0
        while True:
            if dt.datetime.now() - start_time > dt.timedelta(seconds=max_seconds):
                break

            self.max_depth = iteration_depth
            best_reward, best_action_list = self.minimax(player_1, player_2, 0, self.alpha, self.beta)

            iteration_depth += 1

        return best_reward, best_action_list
